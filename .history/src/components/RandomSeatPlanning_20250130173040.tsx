...
const RandomSeatingPlanner: React.FC = () => {
  ...
  const generateSeatingPlan = (): void => {
    ...
    const getAllPositions = () => {
      ...
      seatingStructure.forEach((group, g) => {
        group.benches.forEach((bench, b) => {
          bench.forEach((_, s) => {
            if (!seatingStructure[g].benches[b][s]) {
    ...
    // Modified placement strategy
    const placeStudents = (): boolean => {
      // Sort classes by size (largest first) and convert to array of students
      ...
      let attempts = 0;
      const maxAttempts = 50; // Maximum number of complete restart attempts

      while (attempts < maxAttempts) {
        // Reset seating structure
        seatingStructure.forEach(group => {
          group.benches = Array(group.benches.length)
            .fill(null)
            .map(() => Array(studentsPerBench).fill(null));
        });
        ...
        for (const student of shuffledStudents) {
          ...
          for (const pos of positions) {
            if (isValidPlacement(pos.group, pos.bench, pos.seat, student, seatingStructure)) {
    ...
    const success = placeStudents();

    if (!success) {
      setError(
        "Could not find a valid seating arrangement. Try adjusting the class sizes or adding more seats."
      );
      return;
    }
