...
const RandomSeatingPlanner: React.FC = () => {
  ...
  const generateSeatingPlan = (): void => {
    ...
    // Modified placement algorithm using iterative approach instead of recursion
    const placeStudentsIterative = (students: Student[]): boolean => {
      const unplacedStudents = [...students];
      const maxAttempts = 1000; // Prevent infinite loops
      let attempts = 0;

      while (unplacedStudents.length > 0 && attempts < maxAttempts) {
        const student = unplacedStudents[0];
        let placed = false;

        // Try to find a valid position
        for (let g = 0; g < seatingStructure.length && !placed; g++) {
          for (let b = 0; b < seatingStructure[g].benches.length && !placed; b++) {
            for (let s = 0; s < studentsPerBench && !placed; s++) {
              if (
                seatingStructure[g].benches[b][s] === null &&
                isValidPlacement(g, b, s, student, seatingStructure)
              ) {
                seatingStructure[g].benches[b][s] = student;
        ...
        if (!placed) {
          // If we couldn't place the current student, shuffle remaining students
          // and try again with a different order
          shuffleArray(unplacedStudents);
          attempts++;
        }
      }

      return unplacedStudents.length === 0;
    };
    ...
    // Shuffle students initially for random distribution
    shuffleArray(allStudents);

    // Try to place all students
    const success = placeStudentsIterative(allStudents);

    if (!success) {
      setError(
        "Could not find a valid seating arrangement. Try reducing constraints or adding more seats."
      );
      return;
    }
