...
const RandomSeatingPlanner: React.FC = () => {
  ...
  const generateSeatingPlan = (): void => {
    ...
    const placeStudentsStrategically = (): boolean => {
      ...
      for (let round = 0; round < 3; round++) {  // Do multiple rounds of initial distribution
        for (const classKey of classKeys) {
          if (studentsByClass[classKey].length > 0) {
            ...
            while (tried < maxTries) {
              ...
              if (
                seatingStructure[pos.group].benches[pos.bench][pos.seat] === null &&
                isValidPlacement(pos.group, pos.bench, pos.seat, student, seatingStructure)
              ) {
                seatingStructure[pos.group].benches[pos.bench][pos.seat] = student;
    ...
    const tryRearrangeNearby = (student: Student, structure: GroupStructure[]): boolean => {
        ...
        for (let b = 0; b < structure[g].benches.length; b++) {
          for (let s = 0; s < studentsPerBench; s++) {
            if (structure[g].benches[b][s] === null) {
              ...
              const swapSuccess = trySwapToAccommodate(g, b, s, student, structure);
              if (swapSuccess) {
                return true;
              }
            }
    ...
    const trySwapToAccommodate = (
      ...
      for (let b = Math.max(0, bench - 2); b <= Math.min(structure[group].benches.length - 1, bench + 2); b++) {
        if (b === bench) continue;
        
        const benchStudents = structure[group].benches[b].filter((s): s is Student => s !== null);
        for (const existingStudent of benchStudents) {
          // Temporarily remove student
          structure[group].benches[b][structure[group].benches[b].indexOf(existingStudent)] = null;
          ...
          if (isValidPlacement(group, bench, seat, student, structure)) {
            ...
            for (const pos of allPositions) {
              if (
                ...
                isValidPlacement(pos.group, pos.bench, pos.seat, existingStudent, structure)
              ) {
                structure[pos.group].benches[pos.bench][pos.seat] = existingStudent;
                return true;
              }
            }
          }
